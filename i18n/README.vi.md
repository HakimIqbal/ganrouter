...
<div align="center">
  <img src="../images/ganrouter.png?1" alt="Bảng điều khiển GaNRouter" width="800"/>
  
  # GaNRouter - Free AI Router
  ...
  **Nhà cung cấp AI Miễn cho OpenClaw.**
  ...
  [![npm](https://img.shields.io/npm/v/ganrouter.svg)](https://www.npmjs.com/package/ganrouter)
  [![Downloads](https://img.shields.io/npm/dm/ganrouter.svg)](https://www.npmjs.com/package/ganrouter)
  [![License](https://img.shields.io/npm/l/ganrouter.svg)](https://github.com/HakimIqbal/ganrouter/blob/main/LICENSE)
  
  [🚀 Bắt đầu nhanh](#-quick-start) • [💡 Tính năng](#-key-features) • [📖 Cài đặt](#-setup-guide) • [🌐 Website](https://ganrouter.com)
</div>

---

## 🤔 Tại sao chọn GaNRouter?
...
**GaNRouter giải quyết vấn đề này:**
...
Gói kho lưu trữ này là riêng tư (`ganrouter-app`), vì vậy việc thực thi nguồn/Docker là đường dẫn phát triển cục bộ dự kiến.
...
### 📺 Hướng dẫn thiết lập hoàn chỉnh - GaNRouter + Claude Code MIỄN PHÍ
  
[![Thiết lập GaNRouter + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Xem hướng dẫn từng đầy đủ:**
- ✅ Cài đặt & thiết lập GaNRouter
...
GaNRouter hoạt động liền mạch với tất cả các công cụ code AI chính:
...
- Công cụ CLI của bạn gửi định dạng OpenAI → GaNRouter dịch chuyển → Nhà cung cấp nhận định dạng gốc
...
> Bản thân GaNRouter **không bao giờ thu phí** bạn bất cứ thứ gì. Bạn chỉ trả tiền trực tiếp cho các nhà cung cấp (nếu sử dụng dịch vụ trả phí).
> 
...
### 📊 Hiểu về Chi phí & Thanh toán của GaNRouter

**Thực tế Thanh toán GaNRouter:**

✅ **Phần mềm GaNRouter = MIỄN PHÍ mãi mãi** (mã nguồn mở, không bao giờ thu phí)  
...
❌ **GaNRouter không bao giờ gửi hóa đơn** hoặc tính phí thẻ của bạn
...
- **Nhà cung cấp giá rẻ** (GLM, MiniMax): Trả tiền trực tiếp cho họ, GaNRouter chỉ định tuyến
...
<details>
<summary><b>💳 Tôi có bị GaNRouter tính phí không?</b></summary>

**Không.** GaNRouter là phần mềm miễn phí, mã nguồn mở chạy trên máy tính của chính bạn. Nó không bao giờ tính phí bạn bất cứ thứ gì.

**Bạn chỉ trả tiền:**
- ✅ **Nhà cung cấp gói đăng ký** (Claude Code $20/tháng, Codex $20-200/tháng) → Trả tiền trực tiếp cho họ trên website của họ
- ✅ **Nhà cung cấp giá rẻ** (GLM, MiniMax) → Trả tiền trực tiếp cho họ, GaNRouter chỉ định tuyến yêu cầu của bạn
- ❌ **Bản thân GaNRouter** → **Không bao giờ tính phí bất cứ thứ gì, ever**

GaNRouter là một proxy/router cục bộ. Nó không cóẻ tín dụng của bạn, không thể gửi hóa đơn và không có hệ thống thanh toán. Đó là phần mềm hoàn toàn miễn phí.
...
GaNRouter chỉ định tuyến yêu cầu của bạn đến họ - không có "cạm bẫy" hay thanh toán trong tương lai. Đó là các dịch vụ thực sự miễn phí, và GaNRouter giúp chúng dễ sử dụng với hỗ trợ dự phòng.

**Lưu ý:** số nhà cung cấp gói đăng ký (Antigravity, GitHub Copilot) có thể có các khoảng thời gian dùng thử miễn phí có thể trở thành trả phí sau này, nhưng điều này sẽ được các nhà cung cấp đó thông báo rõ ràng, không phải GaNRouter.
...
3. **Sử dụng nhà cung cấp gói đăng ký cuối cùng:**
   ...
   - GaNRouter giúp tối đa hóa giá trị của chúng thông qua theo dõi hạn mức
...
Cơ chế dự phòng thông minh của GaNRouter ngăn chặn các khoản phí bất ngờ:
...
**Bạn nắm quyền kiểm soát:** Đặt giới hạn chi tiêu cho mỗi nhà cung cấp trong bảng điều khiển, và GaNRouter sẽ tuân thủ chúng.
...
**Mẹo Chuyên nghiệp:** Sử dụng Opus cho các tác vụ phức tạp, Sonnet cho tốc độ. GaNRouter theo dõi hạn mức cho mỗi mô hình!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [từ bảng điều khiển ganrouter]
...
```json
{
  ...
  "anthropic_api_key": "your-ganrouter-api-key"
}
```
...
export OPENAI_API_KEY="your-ganrouter-api-key"
...
> **Lưu ý:** OpenClaw chỉ hoạt động với GaNRouter cục bộ. Sử dụng `127.0.0.1` thay vì `localhost` để tránh các vấn đề phân giải6.
...
```bash
# Clone và cài đặt
git clone https://github.com/HakimIqbal/ganrouter.git
cd ganrouter
npm install
npm run build
...
export DATA_DIR="/var/lib/ganrouter"
export PORT="20128"
export HOSTNAME="0.0.0.0"
export NODE_ENV="production"
export NEXT_PUBLIC_BASE_URL="http://localhost:20128"
export NEXT_PUBLIC_CLOUD_URL="https://ganrouter.com"
...
# Hoặc sử dụng PM2
npm install -g pm2
pm2 start --name ganrouter -- start
pm2 save
pm2 startup
```
...
docker run -d \
  --name ganrouter \
  -p 20128:20128 \
  --env-file /root/dev/ganrouter/.env \
  -v ganrouter-data:/app/data \
  -v ganrouter-usage:/root/.ganrouter \
  ganrouter
```
...
docker logs -f ganrouter
 restart ganrouter
docker stop ganrouter && docker rm ganrouter
```
...
| Biến | Mặc định | Mô tả |
|----------|---------|-------------|
| `JWT_SECRET` | `ganrouter-default-secret-change-me` | Bí mật ký JWT cho cookie xác thực bảng điều khiển (**thay đổi trong production**) |
| `INITIAL_PASSWORD` | `123456` | Mật khẩu đăng nhập đầu tiên khi không có hash đã lưu tồn tại |
| `DATA_DIR` | `~/.ganrouter` |ị trí cơ sở dữ liệu ứng dụng chính (`db.json`) |
...
| `CLOUD_URL` | `https://ganrouter.com` | URL cơ sở endpoint đồng bộ đám mây phía máy chủ |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | URL cơ sở tương thích ngược/công khai (ưu tiên `BASE_URL` cho runtime máy chủ) |
| `NEXT_PUBLIC_CLOUD_URL` | `https://ganrouter.com` | URL đám mây tương thích ngược/công khai (ưu tiên `CLOUD_URL` cho runtime máy chủ) |
...
- Lịch sử sử dụng và log: `~/.ganrouter/usage.json` và `~/.ganrouter/log.txt`, được quản lý bởi `src/lib/usageDb.js`.
- request/translator tùy chọn: `<repo>/logs/...` khi `ENABLE_REQUEST_LOGS=true`.
- Lưu trữ sử dụng hiện tại tuân theo logic đường dẫn `~/.ganrouter` và độc lập với `DATA_DIR`.
...
- Xác minh `CLOUD_URL` trỏ đến endpoint đám mây dự kiến của bạn (ví dụ: `https://ganrouter.com`)
...
- Triệu chứng thường xuất hiện trên endpoint đám mây công khai (`https://ganrouter.com/v1`) cho các lệnh gọi không phát trực tiếp (non-streaming).
...
- Runtime GaNRouter cục bộ bao gồm dự phòng SSE→JSON cho các lệnh gọi không phát trực tiếp khi upstream trả về `text/event-stream`.
...
- `tester/security/test-cloud-openai-compatible.sh`
  - Gửi một yêu cầu tương thích OpenAI trực tiếp đến endpoint đám mây (`https://ganrouter.com/v1/chat/completions`) với mô hình/key được cung cấp.
...
- Endpoint đám mây công khai (`https://ganrouter.com/v1/chat/completions`):
...
- **Website**: [ganrouter.com](https://ganrouter.com)
- **GitHub**: [github.com/HakimIqbal/ganrouter](https://github.com/HakimIqbal/ganrouter)
- **Issues**: [github.com/HakimIqbal/ganrouter/issues](https://github.com/HakimIqbal/ganrouter/issues)

---

## 👥 Người đóng góp

Cảm ơn tất cả những người đã đóng góp giúp GaNRouter tốt hơn!

[![Contributors](https://contrib.rocks/image?repo=HakimIqbal/ganrouter&max=100&columns=20&anon=1)](https://github.com/HakimIqbal/ganrouter/graphs/contributors)

---

## 📊 Star Chart

[![ Chart](https://starchart.cc/HakimIqbal/ganrouter.svg?variant=adaptive)](https://starchart.cc/HakimIqbal/ganrouter)
