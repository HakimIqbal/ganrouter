...
// Check if settings has GaNRouter config
const hasGaNRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["ganrouter"];
};

// Read per-agent models.json and return current model id (without "ganrouter/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["ganrouter"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasGaNRouter: hasGaNRouterConfig(settings),
...
// POST - Update GaNRouter settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `ganrouter/${model}`;

    // Remove all old ganrouter/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("ganrouter/"))
    ...
    // Add fresh ganrouter models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`ganrouter/${m}`] = {};
    });

    // Remove old ganrouter model from each agent in agents.list
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (agent.model?.startsWith("ganrouter/")) {
    ...
    // Update models.providers.ganrouter with all models
    settings.models.providers["ganrouter"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `ganrouter/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove GaNRouter settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove GaNRouter from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["ganrouter"];
    ...
    // Remove ganrouter models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("ganrouter/"));
    ...
    // Reset agents.defaults.model.primary if it uses ganrouter
    if (settings.agents?.defaults?.model?.primary?.startsWith("ganrouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "GaNRouter settings removed successfully",
    });
  } catch (error) {
