...
const hasGaNRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "GaNRouter");
};

const getGaNRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "GaNRouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getGaNRouterEntry(config);
...
// POST - Apply GaNRouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing GaNRouter entry or append
    const idx = config.findIndex((e) => e.name === "GaNRouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove GaNRouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "GaNRouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "GaNRouter removed from Copilot config",
    });
  } catch (error) {
