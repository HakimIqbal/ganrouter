...
// Check if settings has GaNRouter customModels
const hasGaNRouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id === "custom:GaNRouter-0");
};
...
// POST - Update GaNRouter customModels (merge with existing settings)
export async function POST(request) {
  try {
    ...
    // Remove existing GaNRouter config if any
    settings.customModels = settings.customModels.filter(m => m.id !== "custom:GaNRouter-0");

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Add new GaNRouter config
    const customModel = {
      model: model,
      id: "custom:GaNRouter-0",
      index: 0,
...
// DELETE - Remove GaNRouter customModels only (keep other settings)
