...
const hasGaNRouterConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["ganrouter"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["ganrouter"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasGaNRouter: hasGaNRouterConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("ganrouter/") ? config.model.replace(/^ganrouter\//, "") : null,
...
// POST - Apply GaNRouter as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_ganrouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing ganrouter provider entry and its models
    const existingProvider = config.provider["ganrouter"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["ganrouter"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `ganrouter/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `ganrouter/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("ganrouter/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove GaNRouter provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["ganrouter"]?.models) {
      delete config.provider["ganrouter"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["ganrouter"].models).length === 0) {
        delete config.provider["ganrouter"];
        if (config.model?.startsWith("ganrouter/")) delete config.model;
      } else if (config.model === `ganrouter/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["ganrouter"].models);
        config.model = `ganrouter/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire ganrouter provider
      if (config.provider) delete config.provider["ganrouter"];
      if (config.model?.startsWith("ganrouter/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("ganrouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "GaNRouter settings removed from OpenCode",
    });
  } catch (error) {
