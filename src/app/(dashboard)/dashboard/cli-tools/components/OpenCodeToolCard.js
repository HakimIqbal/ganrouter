...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus }) {
  ...
  useEffect(() => {
    ...
    if (status?.config?.agent?.explorer?.model?.startsWith("ganrouter/")) {
      setSubagentModel(status.config.agent.explorer.model.replace("ganrouter/", ""));
    }
  }, [status]);
  ...
  const getConfigStatus = () => {
    ...
    const url = status.config?.provider?.["ganrouter"]?.options?.baseURL || "";
    const isLocal = url.includes("localhost") || url.includes("127.0.0.1");
    return status.hasGaNRouter && (isLocal || url.includes(baseUrl)) ? "configured" : status.hasGaNRouter ? "other" : "not_configured";
  };
  ...
  const getManualConfigs = () => {
    ...
    return [{
      ...
      content: JSON.stringify({
        ...
        model: `ganrouter/${activeModelToShow}`,
        agent: {
          explorer: {
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current base URL */}
                {status?.config?.provider?.["ganrouter"]?.options?.baseURL && (
                  <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!status.hasGaNRouter} loading={restoring}>
