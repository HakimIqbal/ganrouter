...
const WORKER_URL = process.env.TUNNEL_WORKER_URL || "https://ganrouter.com";
const MACHINE_ID_SALT = "ganrouter-tunnel-salt";
...
export async function enableTunnel(localPort = 20128) {
  ...
  if (isCloudflaredRunning()) {
    ...
    if (existing?.tunnelUrl) {
      const publicUrl = `https://r${existing.shortId}.ganrouter.com`;
  ...
  const publicUrl = `https://r${shortId}.ganrouter.com`;
...
export async function getTunnelStatus() {
  ...
  const publicUrl = shortId ? `https://r${shortId}.ganrouter.com` : "";
