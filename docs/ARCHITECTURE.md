# GaNRouter Architecture

_Last updated: 2026-02-06_

## Executive Summary

GaNRouter is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[GaNRouter Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.ganrouter/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.ganrouter/usage.json`, `~/.ganrouter/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.ganrouter/db.json`)
- usage stats: `~/.ganrouter/usage.json`
- request log lines: `~/.ganrouter/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[GaNRouter Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.ganrouter` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/ganrouter && npm run build`
- Build Docker image: `cd /root/dev/ganrouter && docker build -t ganrouter .`
